...
abstract class Base implements \IteratorAggregate
{
        ...
        foreach ($this->parameters as $clauses) {
            if (is_array($clauses)) {
                foreach ($clauses as $value) {
                    if ($this->isNamedParameter($value)) { // for example [':name' => 'Mark']
     ...
     * @return bool
     */
    public function isNamedParameter($value) {
        return is_array($value) && is_string(key($value)) && substr(key($value), 0, 1) == ':';
    }
